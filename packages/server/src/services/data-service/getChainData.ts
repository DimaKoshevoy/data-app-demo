...
export const getTokenPoolLiquidity = async (
  tokens: TokenData[]
  ...
  const tokensWithLiquidityInfo = tokens.filter(
  ...
  const creatorBalancesForTokenWithInfo = tokensWithLiquidityInfo.reduce(
    (acc: any, { tokenAddress }: any, index) => {
      acc[tokenAddress] = {
  ...
  return tokens.reduce((acc: LiquidityData, { tokenAddress }, index) => {
    const [quoteIndex, tokenIndex] = sortsBefore(tokenAddress, QUOTE_CURRENCY)
      ? [1, 0]
      : [0, 1];
    acc[tokenAddress] = {
