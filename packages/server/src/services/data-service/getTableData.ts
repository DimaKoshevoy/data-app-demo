...
const getDataUSD = (data: any) => {
  const reference = data.find(
    ({ tokenAddress }: any) => tokenAddress === USDC || tokenAddress === DAI
  );
  const { quoteReserve, tokenReserve, decimals } = reference;
  const priceInQuote =
    Number(ethers.utils.formatEther(quoteReserve.toString())) /
    Number(ethers.utils.formatUnits(tokenReserve.toString(), decimals));
...
export const getTableData = (data: any) => {
  ...
  const tableData = data.flatMap((tokenVolumeData: any) => {
    ...
    const delta =
      largestIntervalData.buyAmount / largestIntervalData.sellAmount;

    const sinceLaunched = timestamp
      ? (Math.floor(Date.now() / 1000) - timestamp) * 1000
      : 0;
    const week1 =
      sinceLaunched > TIME_INTERVALS_DATA.DAY_1.value
        ? getTimePrice(TIME_INTERVALS.WEEK_1, tokenVolumeData, USD)
        : null;
    const day1 =
      sinceLaunched > TIME_INTERVALS_DATA.HOUR_4.value
        ? getTimePrice(TIME_INTERVALS.DAY_1, tokenVolumeData, USD)
        : null;
    const hour4 =
      sinceLaunched > TIME_INTERVALS_DATA.HOUR_1.value
        ? getTimePrice(TIME_INTERVALS.HOUR_4, tokenVolumeData, USD)
        : null;
    const hour1 =
      sinceLaunched > TIME_INTERVALS_DATA.MINUTE_15.value
        ? getTimePrice(TIME_INTERVALS.HOUR_1, tokenVolumeData, USD)
        : null;
    const minute15 =
      sinceLaunched > TIME_INTERVALS_DATA.MINUTE_15.value
        ? getTimePrice(TIME_INTERVALS.MINUTE_15, tokenVolumeData, USD)
        : null;
    ...
    const creatorLiquidity = creatorBalance
      ? (Number(ethers.utils.formatEther(creatorBalance.toString())) /
          Number(ethers.utils.formatEther(pairTotalSupply.toString()))) *
        100
      : null;
    ...
    return [
      {
        ...
        open: getPriceInUSD(
          USD.usdPrice,
          Number(largestIntervalData.open_price)
        ),
        ...
        close: getPriceInUSD(
          USD.usdPrice,
          Number(largestIntervalData.close_price)
        ),
        minute15,
        hour1,
        hour4,
