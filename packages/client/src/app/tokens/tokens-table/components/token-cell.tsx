...
export const TokenCell = ({value}: any) => {
  ...
  const explorer = (
    <a target="_blank" rel="noreferrer" href={`https://etherscan.com/token/${address}`}>
  ...
  return (
    <div className="truncate">
      ...
      <div className="flex"><div>{symbol}</div> {explorer}<AddressCopy address={address}/> <div className="ml-1">{unlockedLP}</div></div>
    </div>
  )
};
