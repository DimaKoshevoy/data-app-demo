...
const getPercentageTableValue = ({value}: {value: number}) => (
  <div className={`ml-auto truncate overflow-clip ${isNumber(value) ? (value > 0 ? 'text-green-500' : 'text-red-500') : ''}`}>
...
export const TokensTable = React.memo(({data}: Props) => {
  ...
  const onPinClick = useCallback((address: string) => {
    writeStorage('pinned', [...pinned, address]);
  }, [pinned]);

  const onUnpinClick = useCallback((address: string) => {
    writeStorage('pinned', pinned.filter(pinnedAddress => pinnedAddress !== address));
  }, [pinned]);
  ...
  const columns = useMemo(() => [
    {
      ...
      disableSortBy: true,
      width: 36,
      Cell: ({value, row: {values: {tokenAddress}}}: any) => {
        return value ? (
          <EyeOffIcon onClick={() => onUnpinClick(tokenAddress)} className="m-auto h-4 w-4 clickable"/>
        ) : (
          <EyeIcon onClick={() => onPinClick(tokenAddress)} className="m-auto h-4 w-4 clickable"/>
        )
      }
    },
    {
    ...
    },
    {
      ...
      sortDescFirst: true,
      sortType: 'basic',
      Cell: ({value}: any) => <div className="ml-auto">{formatPrice(value)}</div>
    },
    {
      ...
      sortDescFirst: true,
      width: 140,
      Cell: ({value}: any) => <div className="ml-auto">{value.toLocaleString('en-GB')}</div>
    },
    {
      ...
      width: 140,
      sortDescFirst: true,
      Cell: ({value}: any) => <div className="ml-auto">{(Math.floor(value)).toLocaleString('en-GB')}</div>
    },
    {
    ...
    {
      Header: getWrappedHeader(TIME_INTERVALS_DATA[TIME_INTERVALS.MINUTE_15].caption),
      accessor: 'minute15',
      sortDescFirst: true,
      width: 80,
      Cell: getPercentageTableValue
    ...
    {
      Header: getWrappedHeader(TIME_INTERVALS_DATA[TIME_INTERVALS.HOUR_1].caption),
      accessor: 'hour1',
      sortDescFirst: true,
      width: 80,
      Cell: getPercentageTableValue
    ...
    {
      Header: getWrappedHeader(TIME_INTERVALS_DATA[TIME_INTERVALS.HOUR_4].caption),
      accessor: 'hour4',
      sortDescFirst: true,
      width: 80,
      Cell: getPercentageTableValue
    ...
    {
      Header: getWrappedHeader(TIME_INTERVALS_DATA[TIME_INTERVALS.DAY_1].caption),
      accessor: 'day1',
      sortDescFirst: true,
      width: 80,
      Cell: getPercentageTableValue
    ...
    {
      Header: getWrappedHeader(TIME_INTERVALS_DATA[TIME_INTERVALS.WEEK_1].caption),
      accessor: 'week1',
      sortDescFirst: true,
      width: 80,
      Cell: getPercentageTableValue
  ...
  const tableData = useMemo(() => {
    const formattedData =  data.flatMap((tokenData: TokenData) => {
      ...
      const unlockedLP =
        creatorLiquidity && creatorLiquidity > 1 ? <span className="text-yellow-500">{creatorLiquidity.toFixed(1)}% LP owner</span> : null;

      return [{
        pinned: pinned.includes(tokenAddress),
        tokenAddress,
        token: {address: tokenAddress, name: tokenName, symbol: tokenSymbol, unlockedLP},
        launch,
        liquidity,
  ...
  return (
    <div style={{width: totalColumnsWidth}} className="trade-volume-table">
      <div style={{...getTableProps().style, height: TABLE_HEADER_HEIGHT}} className="flex">
